...
function getDb() {
  ...
  if (!_sql) _sql = postgres(url, { ssl: false, max: 1, idle_timeout: 20, connect_timeout: 10 })
  return _sql
}

// Identificador escapado da tabela qualificada (schema.table) — seguro num DB compartilhado.
function tbl(sql: ReturnType<typeof postgres>) {
  return sql(`${SCHEMA}.templates`)
}
...
export async function initDb() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS ${tbl(sql)} (
  ...
  `
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS headline TEXT NOT NULL DEFAULT ''`
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS logo_width INTEGER NOT NULL DEFAULT 170`
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS accent_color TEXT NOT NULL DEFAULT '#A8D156'`
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS template_type TEXT NOT NULL DEFAULT 'modelo01'`
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS logo_href TEXT NOT NULL DEFAULT ''`
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS secondary_color TEXT NOT NULL DEFAULT '#f15a24'`
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS brand_name TEXT NOT NULL DEFAULT ''`
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS handle TEXT NOT NULL DEFAULT ''`
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS footer_logo_url TEXT NOT NULL DEFAULT 'https://pub-db8ed4fb33634589a6ce5fb07e85cb46.r2.dev/logo/op7_dash_odc/logo_op7nexo.svg'`
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS footer_logo_href TEXT NOT NULL DEFAULT 'https://www.instagram.com/op7franquias'`
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS footer_logo_width INTEGER NOT NULL DEFAULT 44`
  await sql`ALTER TABLE ${tbl(sql)} ADD COLUMN IF NOT EXISTS footer_copyright TEXT NOT NULL DEFAULT '© 2026 OP7 Nexo · Todos os direitos reservados'`
}
...
function typeDefaults(type: 'modelo01' | 'modelo02') {
  return type === 'modelo02'
    ? { logo_width: 132, accent_color: '#035dd7', headline: 'Transforme seus resultados.' }
    : { logo_width: 170, accent_color: '#A8D156', headline: 'Cuidar do seu sorriso,' }
}
...
export async function createTemplate(data: TemplateInput): Promise<Template> {
  ...
  const type = data.template_type ?? 'modelo01'
  ...
  const rows = await sql`
    ...
    VALUES (
      ${data.slug}, ${data.name}, ${type},
      ${data.logo_url}, ${data.logo_width ?? def.logo_width}, ${data.logo_href ?? ''},
      ${data.headline ?? def.headline}, ${data.subtitle_html},
      ${data.accent_color ?? def.accent_color}, ${data.secondary_color ?? '#f15a24'},
...
export async function deleteTemplate(id: string): Promise<void> {
  const sql = getDb()
  await sql`DELETE FROM ${tbl(sql)} WHERE id = ${id}`
}
